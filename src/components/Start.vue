		...
		<v-layout align-space-between justify-space-around row fill-height>
					...
					<v-list>
						...
						>
							<template v-slot:activator>
								<v-list-tile>
									<v-list-tile-content>
										<v-list-tile-title class="cp-list-group-title">{{ item.title }}</v-list-tile-title>
									</v-list-tile-content>
								</v-list-tile>
							</template>
							...
							>
								<v-list-tile-content>
									...
									<v-list-tile-sub-title style="opacity:.5;font-size: 75%;">{{subItem.path}}</v-list-tile-sub-title>
								</v-list-tile-content>
			...
			<v-flex d-flex xs12 ripple>
				...
				<v-sheet id="drag-file" elevation="4" :class="dragFileClass">
					...
					<v-layout style="margin:0px 50px;" align-center justify-center fill-height>
						<div style="text-align:center; opacity:.3;">
							<h2>Drag your Coderplay Solution file here to open it!</h2>
						</div>
						<div
			...
			<v-flex v-if="false">
				<div id="div1" v-on:drop="drop($event)" v-on:dragover="allowDrop($event)"></div>
				<br>
				<img
					id="drag1"
	...
	export default Vue.extend({
		...
		methods: {
			...
			drop(ev: DragEvent) {
				...
				(ev.target as Element).appendChild(document.getElementById(
					data
				) as Element);
			},
			// --------------------------------------------------------------------------
