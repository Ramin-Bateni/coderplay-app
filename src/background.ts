...
import { app, BrowserWindow, dialog, ipcMain, Menu, protocol, screen, Tray } from "electron";
import {
  createProtocol,
  installVueDevtools,
...
interface IIpcMainOnEvent { sender: { send: (arg0: string, arg1: any) => void; }; }
...
function createWindow(): void {
  ...
  const screenWorkAreaSize: Electron.Size = screen.getPrimaryDisplay().workAreaSize;
  advancedWin = new BrowserWindow({
    width: appConf.window.advanced.size.width > screenWorkAreaSize.width
          ? Defaults.window.advanced.size.width
          : appConf.window.advanced.size.width,
    height: appConf.window.advanced.size.height > screenWorkAreaSize.height
  ...
  advancedWin.on("resize", (event: any) => {
    ...
    appConfigStore.set("window.advanced.state", advancedWin.isMaximized() ? "maximized" : "normal");
...
app.on("activate", () => {
      ...
      if (advancedWin === null) {
        ...
        ReadyAppConfig(appVersion, forceDefaults).then((resultAppConf: IAppConf) => {
...
function setTrayIcon(): void {
  const iconName: string = process.platform === "win32" ? "coderplay_windows_icon.png" : "coderplay_os_icon.png";
  const iconPath: string = path.join(__dirname, "..\\src\\assets\\tray", iconName);
